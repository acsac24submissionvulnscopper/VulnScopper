...
def load_language_model_vectors(entity_vocab, path):
    """
    Load the PCA vectors for the entities in the entity_vocab from the pickle file at path.
    The loaded vectors are ordered in the same order as the entities in the entity_vocab values.
    """
    with open(path, 'rb') as f:
        entity2pca = pickle.load(f)

    shape = list(entity2pca.values())[0].shape
    entity2pca_ordered = []
    num_entities_embedded = 0
    prev_entity_idx = -1
    for entity in entity_vocab:
        assert entity_vocab[entity] > prev_entity_idx, f'entity_vocab is not ordered by entity index: {entity_vocab[entity]} > {prev_entity_idx}'
        assert entity_vocab[entity] == prev_entity_idx + 1, f'entity_vocab is not ordered by entity index: {entity_vocab[entity]} != {prev_entity_idx} + 1'
        
        prev_entity_idx = entity_vocab[entity].item()
        try:
            entity_embd = torch.tensor(entity2pca[entity])
            entity2pca_ordered.append(entity_embd)
            num_entities_embedded += 1
        except (IndexError, KeyError):
            logger.warn(f'|ADA| Embeddings not found for entity: {entity}, initializing with zeros...')
            entity2pca_ordered.append(torch.zeros(shape))
        except TypeError as e:
            logger.error(f'|ADA| TypeError while getting embedding for entity: {entity}')
            raise e
        
    del entity2pca
    logger.info(f'|ADA| Total entities with pre-trained embeddings = {num_entities_embedded} out of {len(entity_vocab)} entities.')

    # return as float tensor 
    return torch.stack(entity2pca_ordered).float()
