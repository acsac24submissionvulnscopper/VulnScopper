...
class TransductiveDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        # save entity2id and relation2id files
        # merge train, valid, and test entity and relation vocabs
        entity_vocab = {**train_results["inv_entity_vocab"], **valid_results["inv_entity_vocab"], **test_results["inv_entity_vocab"]}
        rel_vocab = {**train_results["inv_rel_vocab"], **valid_results["inv_rel_vocab"], **test_results["inv_rel_vocab"]}

        setattr(train_data, "entity_vocab", entity_vocab)
        setattr(train_data, "rel_vocab", rel_vocab)
        setattr(valid_data, "entity_vocab", entity_vocab)
        setattr(valid_data, "rel_vocab", rel_vocab)
        setattr(test_data, "entity_vocab", entity_vocab)
        setattr(test_data, "rel_vocab", rel_vocab)
...
class RedHatTransductive(TransductiveDataset):

    name = "RedHatTransductive"
    delimiter = '\t'
...
class RedHatOutOfSample(TransductiveDataset):

    name = "RedHatOutOfSample"
    delimiter = '\t'
...
class RedHatFinetune(TransductiveDataset):
    """Fine-tuning dataset for CVE Out-of-sample Datasets (excluding NvdOutOfSample + RedHatOutOfSample datasets)"""
    name = "RedHatFinetune"
    delimiter = '\t'
...
class RedHatWithDates(TransductiveDataset):

    name = "RedHatWithDates"
    delimiter = '\t'
...
class RedHatInductive(InductiveDataset):

    name = "RedHatInductive"
    delimiter = '\t'
...
class NvdOutOfSample(TransductiveDataset):
...
class NvdOutOfSampleSliced(TransductiveDataset):

    name = "NvdOutOfSampleSliced"
    delimiter = '\t'
...
class NvdInductive(InductiveDataset):
...
class JointDataset(InMemoryDataset):
    ...
    datasets_map = {
        ...
        'YAGO310': YAGO310,
        'AristoV4': AristoV4,
        'NvdOutOfSample': NvdOutOfSample,
        'RedHatFinetune': RedHatFinetune
    }
