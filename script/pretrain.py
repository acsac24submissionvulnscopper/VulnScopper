...
if __name__ == "__main__":
    ...
    assert len(train_data) == 1, "Only one graph is allowed for pretraining when LLM integration is set."
    logger.warning(f'LM Vectors are set to: {cfg.train.lm_vectors}')

    model = Ultra(
        rel_model_cfg=cfg.model.relation_model,
        entity_model_cfg=cfg.model.entity_model,
        lm_vectors=util.load_language_model_vectors(train_data[0].entity_vocab, cfg.train.lm_vectors) if cfg.train.lm_vectors else None
    )
