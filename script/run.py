        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in islice(train_loader, batch_per_epoch):
                ...
                if util.get_rank() == 0 and batch_id % cfg.train.log_interval == 0:
                    logger.warning(separator)
                    logger.warning("binary cross entropy: %g batch (%d/%d)" % (loss, i, batch_per_epoch))
...
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False):
    ...
    model.eval()
    metrics = {}
    for relation, test_loader in relation2loader.items():
        metrics[relation] = {}  # reset metrics for each relation
...
if __name__ == "__main__":
    ...
    model = Ultra(
        ...
        lm_vectors=util.load_language_model_vectors(train_data.entity_vocab)
    )
    ...
    if "checkpoint" in cfg and cfg.checkpoint is not None:
        ...
        try:
            ...
            logger.warning("======== No language model vectors in the given checkpoint ========")
